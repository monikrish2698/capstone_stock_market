...
schema = StructType([
    ...
    StructField("ema",    DoubleType(), True)   # the column will be renamed later
])
...
def build_df(results: list, col_name: str):
    ...
    return df.withColumnRenamed("ema", col_name)
...
for ticker in tickers:
    ema_12_day = f"https://api.polygon.io/v1/indicators/sma/{ticker}?timespan=day&adjusted=true&window=12&series_type=close&order=desc&limit=5000"
    ema_26_day = f"https://api.polygon.io/v1/indicators/sma/{ticker}?timespan=day&adjusted=true&window=26&series_type=close&order=desc&limit=5000"
    ema_50_day = f"https://api.polygon.io/v1/indicators/sma/{ticker}?timespan=day&adjusted=true&window=50&series_type=close&order=desc&limit=5000"

    success, data12, error = make_api_request(ema_12_day, params=params)
    success, data26, error = make_api_request(ema_26_day, params=params)
    success, data50, error = make_api_request(ema_50_day, params=params)
    ...
    df12 = build_df(results12, "12_day_ema")
    df26 = build_df(results26, "26_day_ema")
    df50 = build_df(results50, "50_day_ema")
    ...
    print(f"Processed EMA for {ticker}")
